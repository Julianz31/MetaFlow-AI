...
<style>
  *{margin:0;padding:0;box-sizing:border-box;-webkit-font-smoothing:antialiased;}
  ...
  .stars{color:${ACCENT};font-size:44px;letter-spacing:6px;}
  .social{color:rgba(255,255,255,0.92);font-weight:600;font-size:30px;}
  .cta{margin-top:6px;padding:22px 60px;border-radius:100px;font-weight:800;font-size:34px;color:#fff;
  ...
  <div class="footer">
    ...
    <div class="cta">${copy.cta || 'Lo quiero'} →</div>
  </div>
</div></body></html>`;
}
...
async function renderHtml(html) {
  ...
  try {
    ...
    // supersampled → JPEG 1080x1350 (texto nítido, peso razonable)
    return sharp(buf).resize(W, H, { fit: 'cover' }).jpeg({ quality: 90 }).toBuffer();
  } finally {
    await browser.close();
  }
}
