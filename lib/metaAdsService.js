...
const getAccountStats = async (options = {}) => {
    try {
        ...
        const roasFromMeta = getRoasValue(data.purchase_roas);
        const avgRoas = roasFromMeta > 0 ? roasFromMeta.toFixed(2) : '0.00';
        const facturacion = roasFromMeta > 0
            ? parseFloat((roasFromMeta * inversion).toFixed(2))
            : getActionValue(data.action_values);
...
const getBusinessAssets = async (options = {}) => {
    const [pagesResponse, pixels, whatsappNumbers] = await Promise.all([
