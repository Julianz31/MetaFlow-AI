...
function templateDesire(copy, color, w, h, hasProduct) {
  const canvas = createCanvas(w, h);
  const ctx = canvas.getContext('2d');
  ctx.clearRect(0, 0, w, h);
  ...
  const cx = w / 2;
  const pmW = w - 120;
  txt(ctx, copy.b1 || 'Resultados reales y visibles', cx, pillY + 52, 30, 'bold', '#fff', 'center', pmW, false);
  txt(ctx, copy.b2 || 'Fórmula premium comprobada', cx, pillY + 96, 26, 'normal', 'rgba(255,255,255,0.90)', 'center', pmW, false);
  txt(ctx, copy.b3 || 'Amplia satisfacción garantizada', cx, pillY + 134, 22, 'normal', 'rgba(255,255,255,0.76)', 'center', pmW, false);
 ...
 */
function buildTemplate(angle, copy, color, format, hasProduct = false) {
  const { w, h } = DIMS[format] || DIMS.square;
  const fn = TEMPLATE_FNS[angle] || TEMPLATE_FNS.desire;
  return fn(copy || {}, color || '#6366f1', w, h, hasProduct);
}
