...
// Load Inter fonts via file:// — librsvg requires local paths, not data: URIs
const FONT_BOLD_PATH = path.join(__dirname, 'fonts', 'Inter-Bold.ttf');
const FONT_REG_PATH  = path.join(__dirname, 'fonts', 'Inter-Regular.ttf');

let FONTS_STYLE = '';
if (fs.existsSync(FONT_BOLD_PATH) && fs.existsSync(FONT_REG_PATH)) {
  FONTS_STYLE = `<style>
    @font-face{font-family:'Inter';src:url('file://${FONT_BOLD_PATH}')format('truetype');font-weight:700;}
    @font-face{font-family:'Inter';src:url('file://${FONT_BOLD_PATH}')format('truetype');font-weight:900;}
    @font-face{font-family:'Inter';src:url('file://${FONT_REG_PATH}')format('truetype');font-weight:400;}
  </style>`;
} else {
  console.warn('[adTemplates] Inter fonts not found at', FONT_BOLD_PATH);
}
