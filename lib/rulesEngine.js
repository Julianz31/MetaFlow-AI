...
const buildAnalysisPrompt = (campaigns, rules) => {
    ...
    const campaignsText = campaigns.slice(0, 50).map(c =>
        `• ${c.campaign_name} (ID: ${c.campaign_id}) | Estado: ${c.effective_status || c.status} | Gasto: $${c.spend} | ROAS: ${c.roas}x | CPC: $${c.cpc} | CPM: $${c.cpm} | CTR: ${c.ctr}% | Clics: ${c.clicks} | Impresiones: ${c.impressions}`
    ).join('\n');
