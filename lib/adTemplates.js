...
function bullets(ctx, items, x, y, color, textColor, size, spacing, maxWidth) {
  items.forEach((item, i) => {
    ...
    txt(ctx, item, x + 36, cy, size, 'normal', textColor, 'left', maxWidth);
  });
}
...
function templatePain(copy, color, w, h, hasProduct) {
  ...
  // Strong top + bottom dark zones
  fadeRect(ctx, 0, 0, w, h * 0.5, 'rgba(0,0,0,0.92)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.55, w, h * 0.45, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.95)');
  ...
  const headY = wrapText(ctx, copy.headline || '¿Cansado de no ver resultados?',
    w / 2, 88, w - 60, 72, 'bold', color, 84, 'center');

  // Empathy subtext with backdrop
  const subtext = (copy.primaryText || 'Sabemos exactamente como te sientes.').split('.')[0] + '.';
  roundRect(ctx, 30, headY + 4, w - 60, 52, 8, 'rgba(0,0,0,0.45)');
  txt(ctx, subtext, w / 2, headY + 36, 28, 'normal', '#f1f5f9', 'center', w - 80);

  // Bottom text block: bullets (constrained to left half when product is present)
  ...
  const bulletMaxW = hasProduct ? Math.round(w * 0.50) : undefined;
  bullets(ctx, bItems, 36, bY, color, '#fff', 30, 56, bulletMaxW);

  // CTA
  pill(ctx, w / 2 - 220, h - 116, 440, 78, color, copy.cta || 'Ver Solucion', '#fff', 34);
...
function templateDesire(copy, color, w, h, hasProduct) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.5, 'rgba(0,0,0,0.88)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.55, w, h * 0.45, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.95)');

  ctx.fillStyle = color; ctx.fillRect(0, 0, w, 10);
  drawStars(ctx, w / 2 - 5 * 16, 44, 5, 10, color);

  const headY = wrapText(ctx, copy.headline || 'Logra el resultado que siempre quisiste',
    w / 2, 96, w - 60, 70, 'bold', '#ffffff', 82, 'center');

  // Color underline accent
  roundRect(ctx, w / 2 - 100, headY + 6, 200, 6, 3, color);
  ...
  const bulletMaxW = hasProduct ? Math.round(w * 0.50) : undefined;
  bullets(ctx, bItems, 36, Math.round(h * 0.60), color, '#fff', 30, 56, bulletMaxW);
  pill(ctx, w / 2 - 240, h - 116, 480, 78, color, copy.cta || 'Lo quiero ahora', '#fff', 34);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateObjection(copy, color, w, h) {
  ...
  // Left side: slight dark vignette so AI photo shows through
  fadeRect(ctx, 0, 0, half, h, 'rgba(0,0,0,0.38)', 'rgba(0,0,0,0.10)', false);

  // Right side: solid white panel — clean infographic area
  roundRect(ctx, half + 10, 10, half - 20, h - 20, 20, 'rgba(255,255,255,0.97)');

  // LEFT: red/danger header bar
  roundRect(ctx, 10, 10, half - 20, 72, 20, 'rgba(220,38,38,0.90)');
  roundRect(ctx, 10, 40, half - 20, 42, 0, 'rgba(220,38,38,0.90)'); // square bottom
  txt(ctx, '✕', 52, 56, 32, 'bold', '#fff', 'center', undefined, false);
  wrapText(ctx, copy.problemTitle || (copy.headline ? copy.headline.slice(0,36) : '¿Sufres de esto?'),
    80, 38, half - 100, 28, 'bold', '#fff', 40, 'left', false);

  // LEFT: problem bullets (white text on dark)
  const problems = [
    copy.p1 || (copy.primaryText ? copy.primaryText.split('.')[0] : 'Sin resultados después de meses'),
    copy.p2 || 'Dinero gastado sin mejora',
    copy.p3 || 'Frustración y cansancio constante',
  ];
  problems.forEach((p, i) => {
    const py = 108 + i * 72;
    ctx.beginPath(); ctx.arc(34, py + 14, 12, 0, Math.PI * 2);
    ctx.fillStyle = 'rgba(220,38,38,0.85)'; ctx.fill();
    txt(ctx, '✕', 34, py + 19, 16, 'bold', '#fff', 'center', undefined, false);
    wrapText(ctx, p, 58, py, half - 76, 26, 'bold', '#fff', 38, 'left', false);
  });

  // RIGHT: color/brand header bar
  roundRect(ctx, half + 10, 10, half - 20, 72, 20, color);
  roundRect(ctx, half + 10, 40, half - 20, 42, 0, color);
  txt(ctx, '✓', half + 46, 56, 32, 'bold', '#fff', 'center', undefined, false);
  wrapText(ctx, copy.solutionTitle || 'La solución real',
    half + 72, 38, half - 100, 28, 'bold', '#fff', 40, 'left', false);

  // RIGHT: solution bullets (dark text on white)
  const solutions = [
    copy.s1 || 'Resultados comprobados desde la 1ª semana',
    copy.s2 || 'Fórmula premium certificada',
    copy.s3 || 'Miles de clientes satisfechos',
  ];
  solutions.forEach((s, i) => {
    const sy = 108 + i * 72;
    ctx.beginPath(); ctx.arc(half + 30, sy + 14, 12, 0, Math.PI * 2);
    ctx.fillStyle = color; ctx.fill();
    txt(ctx, '✓', half + 30, sy + 19, 16, 'bold', '#fff', 'center', undefined, false);
    wrapText(ctx, s, half + 54, sy, half - 72, 26, 'bold', '#1e293b', 38, 'left', false);
  });

  // Bottom banner
  const bannerY = h - 72;
  roundRect(ctx, 10, bannerY, w - 20, 62, 16, color);
  txt(ctx, (copy.cta || 'PRODUCTO GARANTIZADO · MEJORA VISIBLE EN SEMANAS').toUpperCase(),
    w / 2, bannerY + 38, 24, 'bold', '#fff', 'center', w - 60, false);
...
function templateUrgency(copy, color, w, h) {
  ...
  // Lighter overlays so AI cinematic background shows through
  fadeRect(ctx, 0, 0, w, h * 0.50, 'rgba(0,0,0,0.78)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.50, w, h * 0.50, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.82)');
  ...
  ctx.beginPath();
  ctx.arc(w - 102, 110, 76, 0, Math.PI * 2);
  ctx.fillStyle = color; ctx.globalAlpha = 0.95; ctx.fill(); ctx.globalAlpha = 1;
  txt(ctx, 'SOLO', w - 102, 92, 22, 'bold', '#fff', 'center', undefined, false);
  txt(ctx, 'HOY', w - 102, 120, 22, 'bold', '#fff', 'center', undefined, false);

  // Big bold headline
  wrapText(ctx, copy.headline || 'Oferta por tiempo limitado!',
    w / 2, 96, w - 214, 72, 'bold', '#fff', 84, 'center');

  // Sub-copy
  const sub = copy.description || copy.primaryText?.split('.')[0] || 'Resultados garantizados desde la primera semana';
  txt(ctx, sub, w / 2, Math.round(h * 0.50), 28, 'normal', 'rgba(255,255,255,0.90)', 'center', w - 80);

  // ─── Bottom feature circles (ClickAds-style) ───────────────────────────
  const featureLabels = [
    copy.f1 || 'Resultados\nreales',
    copy.f2 || 'Envío\ngratis',
    copy.f3 || 'Garantía\ntotal',
  ...
  const circR = 50;
  const circleBaseY = Math.round(h * 0.64);
  ...
  featureLabels.forEach((label, i) => {
    ...
    // Inner brand-color fill
    ctx.beginPath(); ctx.arc(cx, circleBaseY, circR - 6, 0, Math.PI * 2);
    ctx.fillStyle = color; ctx.globalAlpha = 0.88; ctx.fill(); ctx.globalAlpha = 1;
    // Label below
    label.split('\n').forEach((line, li) => {
      txt(ctx, line.toUpperCase(), cx, circleBaseY + circR + 22 + li * 26, 20, 'bold', '#fff', 'center', step - 16, false);
    });
  });

  // Pink divider line above circles
  ctx.fillStyle = color; ctx.fillRect(0, circleBaseY - circR - 14, w, 4);

  pill(ctx, w / 2 - 240, h - 116, 480, 78, color, copy.cta || 'Compra ahora!', '#fff', 36);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateAuthority(copy, color, w, h, hasProduct) {
  ...
  // Minimal overlay — let the bright professional AI background show through
  fadeRect(ctx, 0, 0, w, h * 0.30, 'rgba(0,0,0,0.42)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.80, w, h * 0.20, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.60)');

  ctx.fillStyle = color; ctx.fillRect(0, 0, w, 10);

  // Headline on white semi-transparent pill
  roundRect(ctx, 20, 18, w - 40, 106, 16, 'rgba(255,255,255,0.92)');
  wrapText(ctx, copy.headline || 'Formula premium respaldada por expertos',
    w / 2, 52, w - 80, 44, 'bold', '#111827', 58, 'center', undefined, false);

  // Feature panel — right half when product present, full width otherwise
  ...
  roundRect(ctx, panelX, panelY, panelW, panelH, 18, 'rgba(255,255,255,0.93)');

  const feats = [
    copy.f1 || 'Ingredientes certificados de alta calidad',
    copy.f2 || 'Resultados comprobados clinicamente',
    copy.f3 || 'Sin efectos secundarios',
    copy.f4 || 'Estandares de fabricacion premium',
  ];
  const featSpacing = Math.round(panelH / 4.2);
  feats.forEach((f, i) => {
    const fy = panelY + 18 + i * featSpacing;
    // Colored dot
    ctx.beginPath(); ctx.arc(panelX + 20, fy + 11, 9, 0, Math.PI * 2);
    ctx.fillStyle = color; ctx.fill();
    txt(ctx, f, panelX + 38, fy + 15, 22, 'bold', '#111827', 'left', panelW - 50, false);
  });

  pill(ctx, w / 2 - 210, h - 114, 420, 76, color, copy.cta || 'Ver mas', '#fff', 32);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateComparison(copy, color, w, h) {
  ...
  // Light white wash so AI background shows faintly behind clean layout
  ...
  // ── ANTES card (left) ────────────────────────────────────────────────────
  roundRect(ctx, 14, cardY, col - 22, cardH, cardR, '#f8fafc');
  roundRect(ctx, 14, cardY, col - 22, 52, cardR, '#1e293b');
  roundRect(ctx, 14, cardY + 32, col - 22, 20, 0, '#1e293b'); // square bottom of header
  txt(ctx, 'ANTES', 14 + (col - 22) / 2, cardY + 34, 26, 'bold', '#fff', 'center', undefined, false);
  // Description items
  const befores = [copy.b1 || 'Pérdida excesiva', copy.b2 || 'Sin brillo ni vida'];
  befores.forEach((d, i) => {
    const dy = cardY + 72 + i * 62;
    roundRect(ctx, 26, dy, col - 46, 48, 10, 'rgba(15,23,42,0.06)');
    txt(ctx, d, 14 + (col - 22) / 2, dy + 28, 22, 'bold', '#374151', 'center', col - 50, false);
  });

  // ── CENTER: headline + product space ────────────────────────────────────
  const centerX = col + Math.round(col / 2);
  wrapText(ctx, copy.headline || 'TRANSFORMA\nTU VIDA',
    centerX, cardY - 36, col - 10, 26, 'bold', color, 38, 'center', false);
  // Product is composited here by compositeAll (comparison → centered)

  // Tagline below product space
  wrapText(ctx, copy.description || (copy.productName ? copy.productName.toUpperCase() : 'LA SOLUCIÓN'),
    centerX, cardY + Math.round(cardH * 0.70), col - 10, 22, 'bold', '#1e293b', 32, 'center', false);

  // ── DESPUÉS card (right) ─────────────────────────────────────────────────
  roundRect(ctx, col * 2 + 8, cardY, col - 22, cardH, cardR, hexToRgba(color, 0.08));
  roundRect(ctx, col * 2 + 8, cardY, col - 22, 52, cardR, color);
  roundRect(ctx, col * 2 + 8, cardY + 32, col - 22, 20, 0, color);
  txt(ctx, 'DESPUÉS', col * 2 + 8 + (col - 22) / 2, cardY + 34, 26, 'bold', '#fff', 'center', undefined, false);
  const afters = [copy.a1 || 'Pelaje exuberante', copy.a2 || 'Piel sana y vital'];
  afters.forEach((d, i) => {
    const dy = cardY + 72 + i * 62;
    roundRect(ctx, col * 2 + 20, dy, col - 46, 48, 10, hexToRgba(color, 0.10));
    txt(ctx, d, col * 2 + 8 + (col - 22) / 2, dy + 28, 22, 'bold', color, 'center', col - 50, false);
  });

  // Bottom tagline
  const tagline = copy.primaryText ? copy.primaryText.split('.')[0] + '.' : 'Resultados visibles en semanas. Consulta a tu profesional.';
  txt(ctx, tagline, w / 2, cardY + cardH + 38, 22, 'normal', '#6b7280', 'center', w - 60, false);
...
function templateGuarantee(copy, color, w, h, hasProduct) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.48, 'rgba(0,0,0,0.90)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.55, w, h * 0.45, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.95)');
  ctx.fillStyle = color; ctx.fillRect(0, 0, w, 10);

  // Shield badge
  roundRect(ctx, w / 2 - 80, 22, 160, 130, 22, color);
  roundRect(ctx, w / 2 - 58, 38, 116, 96, 14, 'rgba(255,255,255,0.18)');
  txt(ctx, 'GRTIA', w / 2, 108, 34, 'bold', '#fff', 'center', undefined, false);

  txt(ctx, 'Garantia total', w / 2, 190, 58, 'bold', '#fff', 'center');
  txt(ctx, 'de devolucion', w / 2, 246, 40, 'bold', color, 'center');
  pill(ctx, w / 2 - 200, 264, 400, 60, color, '100% GARANTIZADO', '#fff', 28);

  const terms = (copy.primaryText || 'Si no ves resultados en 30 dias te devolvemos tu dinero sin preguntas.').split('.')[0] + '.';
  txt(ctx, terms, w / 2, 352, 24, 'normal', '#cbd5e1', 'center', w - 80);

  // Bullets constrained to left half when product is present on the right
  const bItems = [copy.b1 || 'Devolucion completa si no funciona', copy.b2 || 'Sin preguntas ni tramites', copy.b3 || 'Proceso simple y rapido'];
  const bulletMaxW = hasProduct ? Math.round(w * 0.48) : undefined;
  bullets(ctx, bItems, 36, Math.round(h * 0.61), color, '#fff', 28, 54, bulletMaxW);

  pill(ctx, w / 2 - 230, h - 116, 460, 78, color, copy.cta || 'Compra sin riesgo!', '#fff', 32);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateSocialProof(copy, color, w, h) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.46, 'rgba(0,0,0,0.90)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.54, w, h * 0.46, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.95)');
  ctx.fillStyle = color; ctx.fillRect(0, 0, w, 10);

  txt(ctx, '+10,000', w / 2, 100, 86, 'bold', '#fff', 'center');
  txt(ctx, 'clientes satisfechos', w / 2, 152, 36, 'bold', color, 'center');
  drawStars(ctx, w / 2 - 5 * 18, 200, 5, 13, color);
  txt(ctx, '4.9/5 · Mas de 800 resenas verificadas', w / 2, 236, 24, 'normal', '#94a3b8', 'center');
  ...
  const cardY1 = Math.round(h * 0.56);
  avatarCard(ctx, 24, cardY1, cardW, 106, color, 'C', r1, 'Camila R.');
  avatarCard(ctx, 24, cardY1 + 118, cardW, 106, color, 'D', r2, 'Diego M.');

  pill(ctx, w / 2 - 220, h - 116, 440, 78, color, copy.cta || 'Unete a ellos!', '#fff', 32);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateCuriosity(copy, color, w, h, hasProduct) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.55, 'rgba(0,0,0,0.92)', 'rgba(0,0,0,0.05)');
  fadeRect(ctx, 0, h * 0.55, w, h * 0.45, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.95)');

  // Headline strip with color band
  const headY = wrapText(ctx, copy.headline || 'Sabias que la mayoria lo hace MAL?',
    w / 2, 84, w - 60, 70, 'bold', '#fff', 84, 'center');

  // Color accent bar under headline
  roundRect(ctx, w / 2 - 120, headY, 240, 6, 3, color);

  txt(ctx, copy.description || 'Lo que nadie te habia contado hasta ahora',
    w / 2, headY + 46, 28, 'normal', 'rgba(255,255,255,0.9)', 'center', w - 80);

  // Hints constrained to left half when product is present on the right
  const hintMaxW = hasProduct ? Math.round(w * 0.46) : undefined;
  const hints = [
    copy.h1 || 'El error que todos cometen sin saberlo',
    copy.h2 || 'La solucion que lo cambia todo',
    copy.h3 || 'Resultados que no esperabas posibles',
  ];
  hints.forEach((hint, i) => {
    const hy2 = Math.round(h * 0.62) + i * 58;
    roundRect(ctx, 36, hy2 - 22, 36, 36, 6, color);
    txt(ctx, '>>', 54, hy2, 20, 'bold', '#fff', 'center', undefined, false);
    txt(ctx, hint, 86, hy2, 28, 'normal', '#f1f5f9', 'left', hintMaxW);
  });

  pill(ctx, w / 2 - 250, h - 116, 500, 80, color, copy.cta || 'Descubre el secreto', '#fff', 34);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templatePrice(copy, color, w, h) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.52, 'rgba(0,0,0,0.92)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.55, w, h * 0.45, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.95)');

  ctx.fillStyle = color; ctx.fillRect(0, 0, w, 72);
  txt(ctx, 'OFERTA ESPECIAL · TIEMPO LIMITADO', w / 2, 49, 34, 'bold', '#fff', 'center', undefined, false);

  // Price block
  txt(ctx, 'Precio normal: $150.000', w / 2, 160, 36, 'normal', '#94a3b8', 'center');
  ctx.strokeStyle = '#ef4444'; ctx.lineWidth = 5; ctx.lineCap = 'round';
  ctx.beginPath();
  ctx.moveTo(w * 0.12, 143); ctx.lineTo(w * 0.88, 143);
  ctx.stroke();

  txt(ctx, '50% OFF!', w / 2, 248, 96, 'bold', '#fff', 'center');
  txt(ctx, 'SOLO $75.000', w / 2, 308, 42, 'bold', color, 'center');

  const desc = copy.description || 'Envio gratis + garantia 30 dias incluida';
  txt(ctx, desc, w / 2, Math.round(h * 0.63), 28, 'normal', '#e2e8f0', 'center', w - 80);
  txt(ctx, 'Solo por esta semana · Quedan pocas unidades',
    w / 2, Math.round(h * 0.63) + 48, 26, 'bold', '#f87171', 'center');

  pill(ctx, w / 2 - 250, h - 122, 500, 84, color, copy.cta || 'Aprovecha ahora!', '#fff', 36);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
