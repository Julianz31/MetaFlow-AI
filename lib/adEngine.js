...
function heroScenePrompt(angle, ctx, format, cleanLabel, brandColor, secondaryColor) {
  const mood = HERO_MOOD[angle] || 'premium, cinematic';
  const palette = brandColor
    ? `based on the BRAND COLOR ${brandColor}${secondaryColor && secondaryColor.toLowerCase() !== '#ffffff' ? ` with subtle secondary accents of ${secondaryColor}` : ''}, harmonized with the product label's palette (if they clash, blend both tastefully — the brand color dominates the environment)`
    : `in the same color family as the product's own branding/label`;
...
SCENE: rich SATURATED gradient studio background ${palette} (deep, vibrant, premium — like a brand-color environment), tone adjusted to the mood: ${mood}. Fine glowing sparkles and soft bokeh floating in the air. A few loose elements of the product itself scattered artfully near its base (drops, gummies, petals, kibble — whatever matches: ${ctx}), plus optionally its cap/lid lying beside it. Optionally a soft out-of-focus contextual element behind (happy pet for pet products, serene person for sleep/beauty).
...
async function prepareCreative({ angle, productContext, primaryColor, secondaryColor, productImageBase64, format = 'vertical', existingCopy, adjustmentInstruction, cleanLabel = true, skinIndex = 0, skinId }) {
  ...
  if (arch === 'saleads' || arch === 'producthero') {
    ...
    const hp = heroScenePrompt(angle, productContext, format, cleanLabel, accent, secondaryColor) + adj;
    if (productB64) {
