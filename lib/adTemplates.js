...
function templatePain(copy, color, w, h, hasProduct) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.52, 'rgba(0,0,0,0.75)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.54, w, h * 0.46, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.82)');
  ...
  pill(ctx, w / 2 - 230, h - 120, 460, 82, color, copy.cta || 'Ver Solución', '#fff', 36);
...
function templateDesire(copy, color, w, h, hasProduct) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.50, 'rgba(0,0,0,0.72)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.52, w, h * 0.48, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.82)');

  ctx.fillStyle = color; ctx.fillRect(0, 0, w, 10);

  // Stars + badge
  drawStars(ctx, w / 2 - 5 * 19, 36, 5, 14, color);
  txt(ctx, '★ ALTAMENTE RECOMENDADO', w / 2, 68, 22, 'bold', color, 'center', undefined, false);
  ...
  pill(ctx, w / 2 - 240, h - 120, 480, 82, color, copy.cta || 'Lo quiero ahora', '#fff', 36);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateTransformation(copy, color, w, h) {
  ...
  pill(ctx, w / 2 - 225, h - 118, 450, 80, color, copy.cta || 'EMPIEZA EL CAMBIO', '#fff', 34);
...
function templateUrgency(copy, color, w, h) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.48, 'rgba(0,0,0,0.70)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.48, w, h * 0.52, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.78)');
  ...
  pill(ctx, w / 2 - 242, h - 120, 484, 82, color, copy.cta || '¡Compra ahora!', '#fff', 38);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateAuthority(copy, color, w, h, hasProduct) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.28, 'rgba(0,0,0,0.36)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.78, w, h * 0.22, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.60)');
  ...
  pill(ctx, w / 2 - 220, h - 118, 440, 80, color, copy.cta || 'Ver más', '#fff', 34);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateGuarantee(copy, color, w, h, hasProduct) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.48, 'rgba(0,0,0,0.78)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.55, w, h * 0.45, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.86)');
  ctx.fillStyle = color; ctx.fillRect(0, 0, w, 10);
  ...
  pill(ctx, w / 2 - 240, h - 120, 480, 82, color, copy.cta || '¡Compra sin riesgo!', '#fff', 34);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateSocialProof(copy, color, w, h) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.44, 'rgba(0,0,0,0.78)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.52, w, h * 0.48, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.88)');
  ctx.fillStyle = color; ctx.fillRect(0, 0, w, 10);

  // Big social proof numbers
  txt(ctx, '+10,000', w / 2, 102, 90, 'bold', '#fff', 'center');
  txt(ctx, 'clientes satisfechos', w / 2, 156, 38, 'bold', color, 'center');
  drawStars(ctx, w / 2 - 5 * 20, 204, 5, 15, color);
  txt(ctx, '4.9/5 · Más de 800 reseñas verificadas', w / 2, 242, 26, 'normal', '#94a3b8', 'center');

  // Testimonial cards — bigger text, more readable
  ...
  const cardW = w - 48;
  const cardH = 126;
  const cardY1 = Math.round(h * 0.55);

  // Card 1
  roundRect(ctx, 24, cardY1, cardW, cardH, 16, 'rgba(255,255,255,0.13)', color, 2);
  ctx.beginPath(); ctx.arc(62, cardY1 + cardH / 2, 32, 0, Math.PI * 2);
  ctx.fillStyle = color; ctx.globalAlpha = 0.85; ctx.fill(); ctx.globalAlpha = 1;
  txt(ctx, 'C', 62, cardY1 + cardH / 2 + 10, 28, 'bold', '#fff', 'center', undefined, false);
  const t1 = r1.length > 54 ? r1.slice(0, 54) + '…' : r1;
  wrapText(ctx, t1, 108, cardY1 + 26, cardW - 120, 26, 'bold', '#fff', 36, 'left', false);
  drawStars(ctx, 108, cardY1 + cardH - 30, 5, 9, color);
  txt(ctx, '— Camila R.', 108 + 5 * 14, cardY1 + cardH - 22, 20, 'normal', color, 'left', undefined, false);

  // Card 2
  const cardY2 = cardY1 + cardH + 16;
  roundRect(ctx, 24, cardY2, cardW, cardH, 16, 'rgba(255,255,255,0.13)', color, 2);
  ctx.beginPath(); ctx.arc(62, cardY2 + cardH / 2, 32, 0, Math.PI * 2);
  ctx.fillStyle = color; ctx.globalAlpha = 0.85; ctx.fill(); ctx.globalAlpha = 1;
  txt(ctx, 'D', 62, cardY2 + cardH / 2 + 10, 28, 'bold', '#fff', 'center', undefined, false);
  const t2 = r2.length > 54 ? r2.slice(0, 54) + '…' : r2;
  wrapText(ctx, t2, 108, cardY2 + 26, cardW - 120, 26, 'bold', '#fff', 36, 'left', false);
  drawStars(ctx, 108, cardY2 + cardH - 30, 5, 9, color);
  txt(ctx, '— Diego M.', 108 + 5 * 14, cardY2 + cardH - 22, 20, 'normal', color, 'left', undefined, false);

  pill(ctx, w / 2 - 230, h - 118, 460, 80, color, copy.cta || '¡Únete a ellos!', '#fff', 34);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateCuriosity(copy, color, w, h, hasProduct) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.55, 'rgba(0,0,0,0.80)', 'rgba(0,0,0,0.04)');
  fadeRect(ctx, 0, h * 0.55, w, h * 0.45, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.88)');
  ...
  pill(ctx, w / 2 - 252, h - 118, 504, 82, color, copy.cta || 'Descubre el secreto', '#fff', 36);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templatePrice(copy, color, w, h) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.50, 'rgba(0,0,0,0.80)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.53, w, h * 0.47, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.88)');
  ...
  pill(ctx, w / 2 - 252, h - 124, 504, 86, color, copy.cta || '¡Aprovecha ahora!', '#fff', 38);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
