...
const testSystemUserConnection = async (options = {}) => {
    ...
    // Verify the token is usable (fails fast with a clear error if it's an app token)
    const me = await graphGet('me', { fields: 'id,name' }, options).catch((err) => {
        const code = err.response?.data?.error?.code;
        const msg = err.response?.data?.error?.message || err.message;
        if (code === 190) throw new Error(`Token inválido o expirado: ${msg}`);
        if (code === 2500) throw new Error(
            'El token no es válido para acceder a la API de Ads. ' +
            'Asegúrate de usar un System User Access Token desde Meta Business Manager, ' +
            'no un App Token ni un token de página.'
        );
        throw err;
    });
