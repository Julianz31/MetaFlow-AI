...
function templateObjection(copy, color, w, h) {
  ...
  wrapText(ctx, copy.headline?.slice(0, 44) || '¿Sufre tu mascota de esto?',
    M + 100, M + 22, panelW - 110, 42, 'bold', '#fff', 54, 'left', false);

  const probs = [
    copy.p1 || 'Caída constante del pelaje',
    copy.p2 || 'Pelaje opaco y sin brillo',
    copy.p3 || 'Picazón e irritación',
  ];
  probs.forEach((p, i) => {
  ...
  const sols = [
    copy.s1 || 'Fortalece la piel',
    copy.s2 || 'Aporta brillo radiante',
    copy.s3 || 'Fórmula completa y natural',
  ];
  sols.forEach((s, i) => {
  ...
  txt(ctx, 'PRODUCTO GARANTIZADO · MEJORA VISIBLE EN SEMANAS',
    w / 2, bannerY + 40, 30, 'bold', '#fff', 'center', w - 60, false);
...
function templateComparison(copy, color, w, h) {
  ...
  const headline = copy.headline || 'TRANSFORMA LA VIDA\nDE TU MASCOTA';
  ...
  const befores = [copy.b1 || 'Pérdida excesiva de cabello', copy.b2 || 'Cabello apagado y piel escamosa'];
  befores.forEach((d, i) => {
  ...
  const afters = [copy.a1 || 'Pelaje exuberante y brillante', copy.a2 || 'Piel sana y vital'];
  afters.forEach((d, i) => {
    const dy = cardY + HEAD + 28 + i * 116;
