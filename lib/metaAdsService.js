...
const RATE_WARN_PCT = 65;  // add delay when usage exceeds this
const RATE_BLOCK_PCT = 80; // hard stop when usage exceeds this

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

const parseUsagePct = (headers = {}) => {
    try {
        const adAcct = headers['x-ad-account-usage'];
        const app = headers['x-app-usage'];
        const adPct = adAcct ? (JSON.parse(adAcct).acc_id_util_pct ?? 0) : 0;
        const appPct = app ? Math.max(...Object.values(JSON.parse(app)).map(Number).filter(Number.isFinite)) : 0;
        return Math.max(adPct, appPct);
    } catch {
        return 0;
    }
};

const checkUsage = async (headers = {}) => {
    const pct = parseUsagePct(headers);
    if (pct >= RATE_BLOCK_PCT) {
        throw new Error(`Límite de API de Meta al ${Math.round(pct)}%. Espera unos minutos antes de continuar para evitar bloqueos.`);
    }
    if (pct >= RATE_WARN_PCT) {
        await sleep(600);
    }
};
...
const isRateLimitError = (error) => {
    ...
    return error.response?.status === 429 || code === 17 || code === 80004 || code === 4;
};
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            if (isRateLimitError(error) && attempt < retries) {
                ...
                console.warn(`Meta rate limit hit, reintentando en ${delay}ms (intento ${attempt + 1}/${retries})`);
                await sleep(delay);
                continue;
            }
            throw error;
...
const getAccessToken = (options = {}) => {
    if (!options.accessToken) throw new Error('Se requiere el Access Token del usuario');
    return options.accessToken;
};

const getAdAccountId = (options = {}) => {
    if (!options.adAccountId) throw new Error('Se requiere el Ad Account ID del usuario');
    const rawAdAccountId = options.adAccountId;
...
const graphGet = async (path, params = {}, options = {}) => {
    return withRetry(async () => {
        ...
        await checkUsage(response.headers);
...
const getRoasValue = (purchaseRoas) => {
    ...
    const preferredTypes = ['omni_purchase', 'purchase', 'offsite_conversion.fb_pixel_purchase'];
    for (const type of preferredTypes) {
        const found = purchaseRoas.find(item => item.action_type === type);
        if (found) {
...
const getActionValue = (items = [], actionTypes = PURCHASE_ACTION_TYPES) => {
    ...
    // Para evitar duplicación o triplicación de las compras en Shopify/Meta,
    // priorizamos un único tipo de acción preferida de compra.
    const preferredTypes = ['omni_purchase', 'purchase', 'offsite_conversion.fb_pixel_purchase'];
    for (const type of preferredTypes) {
        if (actionTypes.has(type)) {
            const found = items.find(item => item.action_type === type);
            if (found) {
                const value = parseFloat(found.value || 0);
                if (Number.isFinite(value)) return value;
            }
        }
    }

    // Fallback: Si no se encontró ningún tipo preferido, sumamos cualquier otra acción
    // que coincida con actionTypes pero excluyendo los preferredTypes ya evaluados.
    return items.reduce((total, item) => {
        if (!actionTypes.has(item.action_type)) {
            return total;
        }
        if (preferredTypes.includes(item.action_type)) {
            return total;
        }
...
const getPurchaseCount = (actions = []) => {
    ...
    return actions.reduce((total, item) => {
        ...
        if (preferredTypes.includes(item.action_type)) return total;
        const value = parseFloat(item.value || 0);
        return Number.isFinite(value) ? total + value : total;
    }, 0);
};
...
const getAccountStats = async (options = {}) => {
    try {
        ...
        const response = await graphGet(`${adAccountId}/insights`, {
            date_preset: options.date_preset || DEFAULT_DATE_PRESET,
        ...
        // La facturación real es el valor de compra único filtrado para evitar duplicados
        const facturacion = getActionValue(data.action_values);
        
        // Calculamos el ROAS promedio basado en la facturación y la inversión real,
        // garantizando consistencia absoluta (Facturación / Inversión).
        let roasVal = 0;
        if (inversion > 0) {
            roasVal = facturacion / inversion;
        } else {
            roasVal = getRoasValue(data.purchase_roas);
        }
        
        const avgRoas = roasVal.toFixed(2);
...
const getCampaignInsights = async (options = {}) => {
    try {
        ...
        return (response.data || []).map((campaign) => {
            ...
            const vcv = getActionValue(campaign.action_values);

            let roas = getRoasValue(campaign.purchase_roas);
            if (roas === 0 && spend > 0 && vcv > 0) {
                roas = vcv / spend;
            }
...
const buildAdFromInsight = (ad, insight) => {
    ...
    const vcv = getActionValue(insight.action_values);

    let roas = getRoasValue(insight.purchase_roas);
    if (roas === 0 && spend > 0 && vcv > 0) {
        roas = vcv / spend;
    }
...
const getPixels = async (options = {}) => {
    const adAccountId = getAdAccountId(options);
    const response = await graphGet(`${adAccountId}/adspixels`, {
...
const getBusinessAssets = async (options = {}) => {
    try {
        ...
        // 1. Intentamos obtener el ID del Business Manager asociado a la Ad Account
        const adAccountInfo = await graphGet(adAccountId, {
            fields: 'business'
        }, options).catch(() => null);
        const businessId = adAccountInfo?.business?.id;

        // 2. Ejecutamos las llamadas iniciales
        const [pagesResponse, pixels, whatsappNumbersResult] = await Promise.all([
            graphGet('me/accounts', {
                fields: 'id,name,picture{data{url}},instagram_business_account{id,username,profile_picture_url},whatsapp_number'
            }, options).catch(() => ({ data: [] })),
            getPixels(options),
            getWhatsAppNumbers(options).catch(() => []),
        ]);

        let pages = (pagesResponse.data || []).map((page) => ({
            id: page.id,
            name: page.name,
        ...
        let whatsappNumbers = whatsappNumbersResult || [];

        // 3. Si hay un Business Manager, buscamos assets adicionales propios o de clientes
        if (businessId) {
            console.log(`Business Manager ID detectado: ${businessId}. Buscando páginas y cuentas de WhatsApp adicionales...`);
            
            const [ownedPagesRes, clientPagesRes, ownedWabaRes, clientWabaRes] = await Promise.all([
                graphGet(`${businessId}/owned_pages`, {
                    fields: 'id,name,picture{data{url}},instagram_business_account{id,username,profile_picture_url},whatsapp_number'
                }, options).catch(() => ({ data: [] })),
                graphGet(`${businessId}/client_pages`, {
                    fields: 'id,name,picture{data{url}},instagram_business_account{id,username,profile_picture_url},whatsapp_number'
                }, options).catch(() => ({ data: [] })),
                graphGet(`${businessId}/owned_whatsapp_business_accounts`, {
                    fields: 'id,name,phone_numbers{id,display_phone_number,verified_name}'
                }, options).catch(() => ({ data: [] })),
                graphGet(`${businessId}/client_whatsapp_business_accounts`, {
                    fields: 'id,name,phone_numbers{id,display_phone_number,verified_name}'
                }, options).catch(() => ({ data: [] }))
            ]);

            // Combinar páginas sin duplicados
            const existingPageIds = new Set(pages.map(p => p.id));
            const businessPages = [...(ownedPagesRes.data || []), ...(clientPagesRes.data || [])];
            ...
            // Combinar números de WhatsApp sin duplicados
            const existingPhoneIds = new Set(whatsappNumbers.map(w => w.id));
            const businessWabas = [...(ownedWabaRes.data || []), ...(clientWabaRes.data || [])];
            
            const businessWhatsappNumbers = businessWabas.flatMap((waba) => 
            ...
            for (const phone of businessWhatsappNumbers) {
                if (!existingPhoneIds.has(phone.id)) {
                    whatsappNumbers.push(phone);
                    existingPhoneIds.add(phone.id);
                }
            }
        }
...
const uploadVideoCreative = async (creative, options = {}) => {
    if (creative.videoId) {
        return { type: 'video', id: creative.videoId };
    }
...
const buildCallToAction = (config) => {
    // Si hay whatsappNumber y es OUTCOME_SALES, pero NO hay destinationUrl (campaña exclusiva de WhatsApp)
    if (config.whatsappNumber && config.objective === 'OUTCOME_SALES' && !config.destinationUrl) {
        return {
...
const addAdsBatch = async (payload, campaignId, adsetId, options = {}) => {
    const objective = CAMPAIGN_OBJECTIVES.find(item => item.value === payload.objective);
    if (!objective) throw new Error('Objetivo de campaña no soportado');

    const config = buildCampaignConfig(payload, objective);
    const adAccountId = getAdAccountId(options);
    const generatedCopy = payload.generatedCopy || {};
    const createdAds = [];
    ...
    for (const [index, creative] of payload.creatives.entries()) {
        ...
        createdAds.push({ ad_id: ad.id, creative_id: creativeResponse.id, asset: uploadedCreative, filename: creative.name });
    }

    return createdAds;
};

const scaleCampaignBudget = async (campaignId, scalePct, options = {}) => {
    const adsetsResponse = await graphGet(`${campaignId}/adsets`, {
    ...
    for (const adset of (adsetsResponse.data || [])) {
        ...
        const oldBudget = toNumber(adset.daily_budget);
        const newBudget = Math.round(oldBudget * (1 + scalePct / 100));
        await sleep(300);
        await graphPost(adset.id, { daily_budget: newBudget }, options);
        results.push({ adset_id: adset.id, adset_name: adset.name, old_budget: oldBudget, new_budget: newBudget });
    }
    return results;
};

const testSystemUserConnection = async (options = {}) => {
    if (!options.adAccountId) throw new Error('Se requiere un Ad Account ID');
    const selectedAccountId = options.adAccountId;
