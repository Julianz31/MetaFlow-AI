...
function templateObjection(copy, color, w, h) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.42, 'rgba(0,0,0,0.9)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.52, w, h * 0.48, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.95)');
  ctx.fillStyle = color; ctx.fillRect(0, 0, w, 10);

  const headY = wrapText(ctx, copy.headline || 'Miles ya lo comprobaron y no pueden creerlo',
    w / 2, 80, w - 60, 62, 'bold', '#fff', 74, 'center');
  txt(ctx, 'Lee lo que dicen:', w / 2, headY + 6, 26, 'normal', color, 'center');
  ...
  const cardY2 = cardY1 + 112;

  avatarCard(ctx, 24, cardY1, cardW, 100, color, 'L', t1, 'Laura M.');
  avatarCard(ctx, 24, cardY2, cardW, 100, color, 'C', t2, 'Carlos R.');

  pill(ctx, w / 2 - 230, h - 116, 460, 78, color, copy.cta || 'Pruebalo sin riesgo', '#fff', 32);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateUrgency(copy, color, w, h) {
  ...
  txt(ctx, 'OFERTA ESPECIAL · TIEMPO LIMITADO', w / 2, 46, 32, 'bold', '#fff', 'center', undefined, false);

  // Urgency badge
  ctx.beginPath();
  ctx.arc(w - 110, 170, 78, 0, Math.PI * 2);
  ctx.fillStyle = color; ctx.globalAlpha = 0.95; ctx.fill(); ctx.globalAlpha = 1;
  txt(ctx, 'SOLO', w - 110, 155, 24, 'bold', '#fff', 'center', undefined, false);
  txt(ctx, 'HOY', w - 110, 183, 24, 'bold', '#fff', 'center', undefined, false);

  wrapText(ctx, copy.headline || 'Oferta por tiempo limitado!',
    w / 2, 140, w - 210, 72, 'bold', '#fff', 82, 'center');

  bullets(ctx, [
    copy.description || 'Resultados garantizados',
    'Envio gratis + garantia incluida',
  ], 50, Math.round(h * 0.62), color, '#fff', 32, 56);

  pill(ctx, w / 2 - 250, h - 122, 500, 84, color, copy.cta || 'Compra ahora!', '#fff', 36);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateAuthority(copy, color, w, h, hasProduct) {
  ...
  fadeRect(ctx, 0, 0, w, h * 0.5, 'rgba(0,0,0,0.90)', 'rgba(0,0,0,0)');
  fadeRect(ctx, 0, h * 0.52, w, h * 0.48, 'rgba(0,0,0,0)', 'rgba(0,0,0,0.95)');
  ctx.fillStyle = color; ctx.fillRect(0, 0, w, 10);

  // Badge
  ctx.beginPath();
  ctx.arc(66, 70, 52, 0, Math.PI * 2);
  ctx.fillStyle = color; ctx.globalAlpha = 0.95; ctx.fill(); ctx.globalAlpha = 1;
  drawStars(ctx, 66 - 16, 80, 1, 16, '#fff');

  wrapText(ctx, copy.headline || 'Formula premium respaldada por expertos',
    134, 56, w - 154, 50, 'bold', '#fff', 60, 'left');

  // Features constrained to left half when product is present on the right
  const featMaxW = hasProduct ? Math.round(w * 0.46) : undefined;
  const feats = [
  ...
  feats.forEach((f, i) => {
    const fy = Math.round(h * 0.60) + i * 62;
    roundRect(ctx, 28, fy, 50, 50, 10, color);
    drawCheck(ctx, 53, fy + 25, 13, color);
    txt(ctx, f, 92, fy + 32, 26, 'normal', '#f1f5f9', 'left', featMaxW);
  });

  pill(ctx, w / 2 - 200, h - 116, 400, 78, color, copy.cta || 'Ver mas', '#fff', 32);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
...
function templateComparison(copy, color, w, h) {
  ...
  // Top banner
  ctx.fillStyle = 'rgba(0,0,0,0.85)'; ctx.fillRect(0, 0, w, 76);
  txt(ctx, 'Por que elegirnos?', w / 2, 50, 38, 'bold', '#fff', 'center', undefined, false);

  // Column headers
  roundRect(ctx, 16, 94, half - 32, 56, 10, '#1e293b');
  txt(ctx, 'Otros', half / 2, 131, 28, 'bold', '#9ca3af', 'center', undefined, false);

  const brand = (copy.productName || 'Nosotros').slice(0, 14);
  roundRect(ctx, half + 16, 94, half - 32, 56, 10, color);
  txt(ctx, brand, half + half / 2, 131, 26, 'bold', '#fff', 'center', undefined, false);

  // VS badge
  ctx.beginPath(); ctx.arc(half, 122, 34, 0, Math.PI * 2);
  ctx.fillStyle = '#fff'; ctx.fill();
  txt(ctx, 'VS', half, 131, 24, 'bold', color, 'center', undefined, false);

  // Divider
  ctx.fillStyle = color; ctx.globalAlpha = 0.7;
  ctx.fillRect(half - 2, 165, 4, Math.round(h * 0.58));
  ctx.globalAlpha = 1;

  const leftItems  = [copy.l1 || 'Resultados genericos', copy.l2 || 'Sin soporte', copy.l3 || 'Calidad dudosa'];
  const rightItems = [copy.r1 || 'Resultados comprobados', copy.r2 || 'Soporte personalizado', copy.r3 || 'Calidad premium'];

  crossItems(ctx, leftItems, 22, 220, 26, 58);
  bullets(ctx, rightItems, half + 22, 220, color, '#fff', 26, 58);

  pill(ctx, w / 2 - 220, h - 116, 440, 78, color, copy.cta || 'Elige lo mejor!', '#fff', 32);
  ctx.fillStyle = color; ctx.fillRect(0, h - 10, w, 10);
