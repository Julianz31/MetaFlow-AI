...
function heroScenePrompt(angle, ctx, format) {
  const mood = HERO_MOOD[angle] || 'premium, cinematic';
  return `Use the EXACT product from the provided image as the HERO of a premium advertising scene. Keep its shape, color, dropper and label EXACTLY — fully readable, do NOT redesign or rewrite the label.
SCENE: cinematic gradient studio background (${mood}) whose palette complements the product's branding, soft glow, subtle bokeh. Mood/props derived from: ${ctx} (for pet products a happy pet softly out of focus; for beauty soft petals; etc.) — minimal and premium.
PLACEMENT: product CENTERED to slightly right, height ~50% of frame, base no lower than 78%, standing on a glossy reflective surface with soft contact shadow and gentle reflection. Relight to match the scene.
COMPOSITION: keep the TOP 26%, the LEFT 40% and the BOTTOM 24% darker/cleaner/uncluttered for text overlays. ${fmtHint(format)}
QUALITY: ${TECH} STRICT: NO marketing text, NO words, NO extra logos beyond the product's own label, NO watermarks, only ONE instance of the product.`;
}
...
function buildProductHero(angle, copy, sceneB64, ACCENT, W, H) {
  const c = copy || {};
  const wrapAccent = (h, a) => a ? String(h || '').replace(new RegExp(`(${String(a).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'i'), '<span class="ac">$1</span>') : (h || '');
  ...
  return `${HEAD}<style>${baseCss(W, H, ACCENT)}
    ...
    .head{font-family:'Poppins';font-weight:800;color:#fff;font-size:72px;line-height:1.02;letter-spacing:-1px;text-shadow:0 4px 24px rgba(0,0,0,0.55);}
    .head .ac{color:${ACCENT};}
    .sub{color:rgba(255,255,255,0.92);font-weight:600;font-size:30px;margin-top:16px;}
    .bens{position:absolute;left:54px;bottom:200px;z-index:6;display:flex;flex-direction:column;gap:20px;max-width:60%;}
    .ben{display:flex;align-items:center;gap:16px;color:#fff;font-weight:800;font-size:36px;text-shadow:0 2px 12px rgba(0,0,0,0.75);}
...
const ARCHETYPE = {
  pain: 'producthero', desire: 'producthero', urgency: 'producthero', authority: 'producthero',
  guarantee: 'producthero', curiosity: 'producthero', price: 'producthero',
...
function buildHtmlFor(angle, copy, scenes, productB64, accent, W, H) {
  const arch = ARCHETYPE[angle] || 'producthero';
  ...
  return buildProductHero(angle, copy, scenes[0], accent, W, H);
}
...
async function prepareCreative({ angle, productContext, primaryColor, productImageBase64, format = 'vertical', existingCopy, adjustmentInstruction }) {
  ...
  let sceneTasks;
  if (arch === 'producthero') {
    // 1 escena: producto como estrella (integrado por Gemini)
    const hp = heroScenePrompt(angle, productContext, format) + adj;
    sceneTasks = [productB64 ? withProduct(hp) : sceneImage(hp)];
  } else {
    ...
    sceneTasks = prompts.map((p, i) =>
      i === prodIdx ? withProduct(p + PRODUCT_INTEGRATE) : sceneImage(productB64 ? p + NO_PRODUCT : p));
  }
