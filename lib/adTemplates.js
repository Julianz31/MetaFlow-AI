...
function templateObjection(copy, color, w, h) {
  ...
  topRound(ctx, M, M, panelW, HEAD, R, DANGER);
  ...
  topRound(ctx, rx, M, panelW, HEAD, R, color);
...
function templateAuthority(copy, color, w, h, hasProduct) {
  ...
  // Feature panel — right side when product present
  const panelX = hasProduct ? Math.round(w * 0.52) : 20;
  const panelW = hasProduct ? w - panelX - 20 : w - 40;
  const panelY = Math.round(h * 0.49);
...
function templateComparison(copy, color, w, h) {
  ...
  topRound(ctx, lx, cardY, cw, HEAD, R, DARK);
  ...
  wrapText(ctx, centerLabel, cx + ccw / 2, cardY + Math.round(cardH * 0.72), ccw - 20, 28, 'bold', DARK, 38, 'center', false);

  // ── RIGHT CARD (DESPUÉS) ──────────────────────────────────────────────────
  const rx2 = col * 2 + M;
  roundRect(ctx, rx2, cardY, cw, cardH, R, hexToRgba(color, 0.08));
  topRound(ctx, rx2, cardY, cw, HEAD, R, color);
